...
class EdgeFeatureGraphCRF(GraphCRF):
    ...
    Pairwise potentials are symmetric and are independent for each kind of
    edges. This leads to n_classes * n_features parameters for unary potentials
    and n_edge_features * n_classes parameters for edge potentials.

    Unary evidence is given as a tuple of shape (n_nodes, n_features),
    An instance ``x`` is represented as a tuple ``(unaries, edges)``.
    Edges is a list of arrays of shape (n_edges, 2), where the first
    array are array of edges of shape (n_edges, 2), and the second
    array are a edge features between those edges of shape 
    ...
    def get_pairwise_potentials(self, x, w):
        #self._check_size_w(w)
        #self._check_size_x(x)
        edges, edge_features = self.get_edges(x)
        pairwise_params = w[self.n_states * self.n_features:]
        edge_weights = edge_features * np.mat(pairwise_params).T
