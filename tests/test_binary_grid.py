...
def test_binary_blocks_subgradient_online():
    #testing subgradient ssvm on easy binary dataset with online learning
    X, Y = toy.generate_blocks(n_samples=10)
    crf = GridCRF()
    clf = SubgradientStructuredSVM(problem=crf, max_iter=200, C=10000,
                                   verbose=10, momentum=.9, learning_rate=0.1,
                                   batch=False)
    clf.fit(X, Y)
    Y_pred = clf.predict(X)
    assert_greater(3, np.sum(Y - Y_pred))
